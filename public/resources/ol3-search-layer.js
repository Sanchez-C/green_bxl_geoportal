  ...
  var returnHorsey = function(input, source, map, select, options) {
    horsey(input, {
      ...
      predictNextSearch: function(info) {
        ...
        var buffer = 1000; // Ajuste cette valeur en fonction de la taille du buffer souhaité
        var bufferedExtent = [
          extent[0] - buffer, // minX
          extent[1] - buffer, // minY
          extent[2] + buffer, // maxX
          extent[3] + buffer  // maxY
        ];
        if (typesToZoomToCenterAndZoom.indexOf(featType) !== -1) {
          ...
          map.getView().fit(bufferedExtent, {size: map.getSize(), duration: 500 });
        }
